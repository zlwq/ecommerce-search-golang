...
const (
	collectionName    = "products_minilm"
...
func main() {
	...
	if qdrantHost == "" || qdrantAPIKey == "" {
		log.Fatal("QDRANT_HOST and QDRANT_API_KEY environment variables must be set")
	}
	...
	if err := streamAndUpsert(ctx, db, qdrantClient, numWorkers); err != nil {
...
func streamAndUpsert(ctx context.Context, db *sql.DB, qdrantClient *qdrant.Client, numWorkers int) error {
